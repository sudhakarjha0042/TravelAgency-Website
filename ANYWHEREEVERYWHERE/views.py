...
from .models import Rating, Location, Review
...
@login_required
def save_review(request,location,review):
        print(location, review)
        user = request.user
        review = Review(location=location, rating=4, review_text=review, user=user)
        review.save()
        reviews = Review.objects.filter(location='JEWELS OF NAVI-MUMBAI').first()
        print(reviews)
        return JsonResponse({'success': True})